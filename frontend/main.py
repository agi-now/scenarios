...
def metadata_extractor(markdown_file):
    lock = 0
...
for file_path, pathandname in zip(files_paths,zip(scenarios_paths,scenarios_names)):
    with open(file_path, 'r') as file:
        scenario_AST = markdown(clean(file.read()),renderer="ast")
    if metadata_extractor(scenario_AST):
        SCE_data, SCE_metadata = metadata_extractor(scenario_AST)
...
for file_path in files_paths:
    with open(file_path, 'r') as file:
        problem_AST = markdown(clean(file.read()),renderer="ast")
    if metadata_extractor(problem_AST):
        PRO_data, PRO_metadata = metadata_extractor(problem_AST)
...
for file_path in files_paths:
    with open(file_path, 'r') as file:
        idea_AST = markdown(clean(file.read()),renderer="ast")
    if metadata_extractor(idea_AST):
        IDE_data, IDE_metadata = metadata_extractor(idea_AST)
...
for i in range(len(scenarios)):
    ...
    for problem in problems:
        if problem[0]["id"] in placeholder:
            ...
            Automator.append({"id":problem[0]["id"],"description":problem[1],"title":problem[0]["title"],"ideas":ideas_md})

    with open('frontend\scenario_template.html', 'r') as file:
        template_content = file.read()

    environment = Environment(loader=FileSystemLoader("frontend"))
    template = environment.get_template("scenario_template.html")
    output = template.render(scenario_content=SCE_data,problems=Automator,scenario_name=output_scenarios_names[i])
